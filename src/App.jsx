...
import { Sparkles, Camera, Palette, Zap, Wallet, Download, Check, X, ChevronDown, Lock } from "lucide-react";
    ...
    <div className="min-h-screen bg-black p-4" style={{ backgroundImage: "radial-gradient(circle at 50% 50%, #1a1a1a 0%, #000 100%)" }}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          ...
          <div className="flex gap-3">
            <button 
              ...
              className="px-4 py-2 rounded-lg font-semibold text-white flex items-center gap-2 transition-all"
              style={{ 
            ...
            {wallet ? (
              ...
              <button 
                ...
                className="px-6 py-2 rounded-lg font-bold text-black flex items-center gap-2 transition-all hover:scale-105"
                style={{ 
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-5">
            ...
            >
              ...
              <button 
                ...
                className="w-full py-4 rounded-lg font-bold text-black transition-all hover:scale-105"
                style={{ 
                  background: "linear-gradient(135deg, #ffd700, #d4af37)",
              ...
              {media && (
                <div className="mt-3 p-3 rounded-lg text-green-400 text-sm flex items-center gap-2" style={{ background: "rgba(74, 222, 128, 0.1)", border: "1px solid #4ade80" }}>
            ...
            >
              <h3 className="text-yellow-400 font-bold mb-4 text-lg">NFT Details</h3>
              <input 
                type="text" 
                value={name} 
                onChange={e => setName(e.target.value)}
                className="w-full bg-black text-white px-4 py-3 rounded-lg mb-4 font-semibold"
              ...
              <div className="grid grid-cols-2 gap-4">
                <select 
                  ...
                  className="bg-black text-white px-4 py-3 rounded-lg font-bold"
                  style={{ 
                    border: "1px solid #333",
                ...
                <div className="flex items-center gap-3 bg-black px-4 rounded-lg" style={{ border: "1px solid #333" }}>
                  <Palette className="w-5 h-5 text-gray-400" />
                  <input type="color" value={color} onChange={e => setColor(e.target.value)} className="flex-1 h-8" />
                </div>
              </div>
            </div>
            ...
            >
              <div className="flex justify-between mb-4">
                <h3 className="text-yellow-400 font-bold text-lg">Attributes</h3>
                <button 
                  onClick={randomizeStats}
                  className="text-yellow-400 text-sm flex items-center gap-1 font-semibold hover:text-yellow-300 transition-colors"
                >
            ...
            >
              ...
              <button 
                ...
                className="w-full py-4 rounded-lg font-bold mb-3 flex items-center justify-center gap-2 transition-all hover:scale-105"
                style={{ 
                  ...
                  boxShadow: txStatus === "pending" ? "none" : isPaid ? "0 4px 20px rgba(34, 197, 94, 0.4)" : "0 4px 20px rgba(212, 175, 55, 0.4)",
                  color: isPaid ? "#fff" : "#000"
                }}
              >
                {isPaid ? (
                  <>
                    <Check className="w-5 h-5" /> Payment Complete
                  </>
                ) : txStatus === "pending" ? (
                  "Processing..."
                ) : (
                  `Pay ${network.fee} ${network.symbol} & Mint`
                )}
              </button>
              <div className="grid grid-cols-2 gap-3">
                <button 
                  ...
                  disabled={!isPaid}
                  className="py-3 rounded-lg font-bold text-white flex items-center justify-center gap-2 transition-all relative"
                  style={{ 
                    background: isPaid 
                      ? "linear-gradient(135deg, #3b82f6, #2563eb)"
                      : "linear-gradient(135deg, #333, #1a1a1a)",
                    boxShadow: isPaid ? "0 4px 20px rgba(59, 130, 246, 0.3)" : "none",
                ...
                <button 
                  ...
                  disabled={!isPaid}
                  className="py-3 rounded-lg font-bold text-white flex items-center justify-center gap-2 transition-all"
                  style={{ 
                    background: isPaid
                      ? "linear-gradient(135deg, #a855f7, #9333ea)"
                      : "linear-gradient(135deg, #333, #1a1a1a)",
                    boxShadow: isPaid ? "0 4px 20px rgba(168, 85, 247, 0.3)" : "none",
                    opacity: isPaid ? 1 : 0.5,
                    cursor: isPaid ? "pointer" : "not-allowed"
                  }}
                >
          ...
          >
            <h3 className="text-yellow-400 font-bold mb-4 text-lg">Live Preview</h3>
            <div className="flex justify-center">
              ...
              >
                ...
                >
                  <h2 className="text-white text-2xl font-bold mb-2" style={{ color }}>{name}</h2>
                  <span 
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div className="bg-black/60 rounded-lg px-3 py-2 font-semibold text-white">⚔️ {stats.attack}</div>
                    <div className="bg-black/60 rounded-lg px-3 py-2 font-semibold text-white">🛡️ {stats.defense}</div>
                    <div className="bg-black/60 rounded-lg px-3 py-2 font-semibold text-white">⚡ {stats.speed}</div>
                    <div className="bg-black/60 rounded-lg px-3 py-2 font-semibold text-white">✨ {stats.magic}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              {WALLETS.map(w => (
                <button
                  ...
                  className="w-full p-4 rounded-xl font-bold text-white flex items-center gap-4 transition-all hover:scale-105"
                  style={{ 
      ...
      {showNetworkModal && (
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4" style={{ background: "rgba(0, 0, 0, 0.85)" }}>
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-yellow-400">Select Network</h2>
            ...
            <div className="space-y-3">
              {NETWORKS.map(n => (
                <button
                  ...
                  className="w-full p-4 rounded-xl font-bold text-white flex items-center justify-between transition-all hover:scale-105"
                  style={{ 
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-lg">{n.name}</span>
                  </div>
                  <span className="text-sm text-gray-400">{n.fee} {n.symbol}</span>
                </button>
              ))}
            </div>
          </div>
