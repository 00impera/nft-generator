...
export default function NFTCardGenerator() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900 p-3">
      {/* ...MODALS AND HEADER OMITTED FOR BREVITY... */}
      <div className="max-w-5xl mx-auto">
        {/* Header and Controls omitted for brevity */}
        <div className="grid md:grid-cols-2 gap-4">
          {/* Controls column omitted for brevity */}
          {/* Preview & Download */}
          <div className="bg-gray-800/50 rounded-lg p-3 border border-gray-700 flex flex-col items-center">
            <h3 className="text-white font-semibold mb-2 text-sm flex items-center gap-2">
              <Film className="w-4 h-4" />
              Preview
            </h3>
            <div className="rounded-lg overflow-hidden mb-3" style={{ border: `2px solid ${color}` }}>
            ...
            <div className="flex gap-2 w-full">
              <button onClick={downloadCard} className={`flex-1 py-2 rounded-lg font-bold ${isPaid ? 'bg-blue-500 hover:bg-blue-600' : 'bg-gray-600'} text-white flex items-center justify-center gap-1`} disabled={!isPaid}>
                <Download className="w-4 h-4" /> PNG
              </button>
              <button onClick={downloadMetadata} className={`flex-1 py-2 rounded-lg font-bold ${isPaid ? 'bg-purple-500 hover:bg-purple-600' : 'bg-gray-600'} text-white flex items-center justify-center gap-1`} disabled={!isPaid}>
                <Download className="w-4 h-4" /> JSON
              </button>
            </div>
            {!isPaid && (
              <button onClick={mintWithPayment} className="mt-3 w-full bg-gradient-to-r from-cyan-500 to-purple-600 hover:from-cyan-600 hover:to-purple-700 text-white font-bold py-2 rounded-lg flex items-center justify-center gap-2">
                <CreditCard className="w-4 h-4" />
                Pay {net.fee} {net.symbol} (Thirdweb)
              </button>
            )}
            {lastTxHash && (
              <a
                href={net.explorer + lastTxHash}
                target="_blank"
